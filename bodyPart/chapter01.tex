...
\textbf{swjtuThesis正是为了能够让更多的交大学生接触并了解~\LaTeX{}，同时能够更为方便地使用~\LaTeX{}~进行研究生学位论文撰写的西南交通大学研究生学位论文~\LaTeX{}~模板}。本模板严格依照《西南交通大学研究生学位论文撰写规范》\footnote{详细信息请参阅西南交通大学研究生院网站页面：\url{https://gsnews.swjtu.edu.cn/info/1102/4733.htm}}进行开发，但由于个人水平有限，难免存在不足之处，欢迎大家积极反馈在使用过程中遇到的问题，或者是对swjtuThesis模板的开发建议，同时也希望能在众多交大~\LaTeX{}~爱好者的努力下，一同完善本模板，受益更多的交大同学。


\section{模板获取}

swjtuThesis模板的所有源代码和资源均作为开源项目的形式托管于GitHub网站，链接地址如下：

\begin{center}
	\url{https://github.com/Pungjay/swjtuThesis}
\end{center}
...
\textbf{对于~\TeX~发行版的选择}，Windows操作系统下推荐使用CTeX中文套装(最新版本：CTeX 3.1.218\footnote{CTeX下载地址：\url{http://www.ctex.org/CTeXDownload}})或者TeX Live套装(最新版本：TeX Live 2024\footnote{TeX Live下载地址：\url{https://www.tug.org/texlive/}})；Linux系统下同样推荐使用TeX Live套装(最新版本：TeX Live 2024)；Mac系统下推荐使用MacTeX(最新版本：MacTeX-2024\footnote{MacTeX下载地址：\url{https://tug.org/mactex/}})。
...
\textbf{swjtuThesis模板所采用的开发环境为}：TeX Live 2023 + TeXStudio。
...
\begin{itemize}
	...
	\item \textbf{swjtuThesis.cls}：文档类文件，基于ctexbook文档类修改，\textbf{不建议用户修改}；
        \item \textbf{swjtuThesis.sty}：文档类文件的样式文件，用于导入各种宏包及其样式设定，\textbf{用户可根据需要修改}；
	\item \textbf{swjtuThesis.cfg}：文档类文件的配置文件，用于字符串定义，\textbf{不建议用户修改}；
\end{itemize}

此外，在使用过程中将会产生如.aux、.bbl、.log、.out等后缀的文件，均属于~\LaTeX{}~编译过程的缓存文件，用户可以双击启动\textbf{delete-temp-files.bat}自动删除。
...
\begin{table}[htbp]
	...
	\begin{tabular}{llll}
		...
		文件夹 & 文件 & 作用说明 & 使用说明 \\
		\midrule
		\multirow{6}{*}{\centering frontPart\textbackslash{}:}
            & coverInformation.tex & 中英文封面信息 & \textbf{用户撰写} \\
            & copyright.tex & 学位论文版权授权书 & \textbf{无需修改} \\
		...
		\midrule
		\multirow{2}{*}{\centering bodyPart\textbackslash{}:} & chap0N.tex & 第N章内容 & \textbf{用户撰写} \\
		& conclusion.tex & 结论 & \textbf{用户撰写} \\
		\midrule
		\multirow{3}{*}{\centering behindPart\textbackslash{}:} & remerciement.tex & 致谢 & \textbf{用户撰写} \\
		& appendixA.tex & 附录A & \textbf{用户撰写} \\
		& myWork.tex & 科研成果 & \textbf{用户撰写} \\
		\midrule
		\multirow{2}{*}{\centering references\textbackslash{}:} & chinesebst.bst & 默认的参考文献样式文件 & \textbf{无需修改} \\
		& swjtuThesis.bst & 本模板所用的参考文献样式文件 & \textbf{无需修改} \\
		& ref.bib & 参考文献~\BibTeX~数据库 & \textbf{用户录入}\\
		\midrule
